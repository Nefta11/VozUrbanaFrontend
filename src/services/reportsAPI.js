...
export const reportsAPI = {
  ...
  async getCategories() {
    try {
      ...
      return [
        { id: 1, nombre: "Saneamiento", icono: "Droplets", descripcion: "Problemas de agua y drenaje", activa: true, orden_visualizacion: 1 },
        { id: 2, nombre: "Infraestructura", icono: "Construction", descripcion: "Calles, banquetas y construcción", activa: true, orden_visualizacion: 2 },
        { id: 3, nombre: "Salud Pública", icono: "Heart", descripcion: "Problemas de salud comunitaria", activa: true, orden_visualizacion: 3 },
        { id: 4, nombre: "Seguridad", icono: "Shield", descripcion: "Seguridad pública y vandalismo", activa: true, orden_visualizacion: 4 },
        { id: 5, nombre: "Medio Ambiente", icono: "Leaf", descripcion: "Contaminación y áreas verdes", activa: true, orden_visualizacion: 5 },
        { id: 6, nombre: "Servicios Públicos", icono: "Zap", descripcion: "Electricidad y servicios básicos", activa: true, orden_visualizacion: 6 },
        { id: 7, nombre: "Transporte", icono: "Bus", descripcion: "Transporte público y vialidad", activa: true, orden_visualizacion: 7 },
        { id: 8, nombre: "Otros", icono: "AlertCircle", descripcion: "Otros problemas urbanos", activa: true, orden_visualizacion: 8 },
      ]
    }
  },
  ...
  async getAll() {
    try {
        ...
        response.data.map(async (report) => {
          try {
            ...
            const [votesResponse, commentsResponse] = await Promise.all([
              apiClient.get(`/votos/${report.id}`).catch(() => ({ data: { up: 0, down: 0, total: 0 } })),
              apiClient.get(`/comentarios/${report.id}`).catch(() => ({ data: [] }))
            ])
      ...
      const message = error.response?.data?.message || 'Error al obtener reportes'
  ...
  async getById(id) {
    try {
      ...
      // Obtener votos y comentarios en paralelo
      const [votesResponse, commentsResponse] = await Promise.all([
        apiClient.get(`/votos/${id}`).catch(() => ({ data: { up: 0, down: 0, total: 0 } })),
      ...
      const message = error.response?.data?.message || "Error al obtener reporte"
      throw new Error(message)
    }
  },
  ...
  async update(id, reportData) {
    try {
      ...
      if (reportData.descripcion) updateData.descripcion = reportData.descripcion
      if (reportData.categoria) updateData.categoria_id = parseInt(reportData.categoria)
      ...
      const message = error.response?.data?.message || "Error al actualizar reporte"
      throw new Error(message)
    }
  },
  ...
  async getByLocation(lat, lng, radius = 0.01) {
    try {
      const response = await apiClient.get(`/reports/location/${lat}/${lng}/${radius}`)
      ...
      const message = error.response?.data?.message || 'Error al obtener reportes por ubicación'
      throw new Error(message)
    }
  },
  ...
  async addComment(reportId, commentData) {
    try {
      ...
      const message = error.response?.data?.message || 'Error al añadir comentario'
      throw new Error(message)
    }
  },
  ...
  async updateComment(commentId, texto) {
    try {
      const response = await apiClient.patch(`/comentarios/${commentId}`, { texto })
      return {
        success: true,
